...
async def _execute_bitbucket(action: str, params: dict, creds: dict) -> dict:
    ...
    async with httpx.AsyncClient(base_url="https://api.bitbucket.org/2.0", timeout=15, headers=headers) as c:
        if action == "create_pr":
...
_SERVICE_HANDLERS = {
    ...
    "dropbox":         _execute_dropbox,
    "box":             _execute_box,
    "bitbucket":       _execute_bitbucket,
    "figma":           _execute_figma,
    "spotify":         _execute_spotify,
    "twitch":          _execute_twitch,
    "fitbit":          _execute_fitbit,
    "freshbooks":      _execute_freshbooks,
    "digitalocean":    _execute_digitalocean,
    "basecamp":        _execute_basecamp,
    "huggingface":     _execute_huggingface,
}
