...
function Step({ n, title, active }: { n: number; title: string; active?: boolean }) {
  return (
    <div className={`flex items-center gap-3 mb-4 ${active ? "" : "opacity-60"}`}>
      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${active ? "bg-zinc-900 text-white" : "bg-zinc-200 text-zinc-500 dark:text-zinc-400"}`}>
        {n}
      </div>
...
function FlowStep({ done, active, label, sub }: { done: boolean; active?: boolean; label: string; sub?: string }) {
  return (
    <div className="flex items-start gap-3">
      <div className={`mt-0.5 w-3 h-3 rounded-full shrink-0 ${done ? "bg-green-50 dark:bg-green-950/300" : active ? "bg-blue-50 dark:bg-blue-950/300 animate-pulse" : "bg-zinc-200"}`} />
      <div>
        <span className={`text-sm ${done ? "text-zinc-800 dark:text-zinc-200 font-medium" : active ? "text-blue-600 font-medium" : "text-zinc-400"}`}>{label}</span>
  ...
  return (
    <div className="max-w-3xl">
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-4">
          <div>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">Install once:</p>
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-3 flex-wrap">
            <div className="flex-1 min-w-0">
              <label className="text-xs text-zinc-500 dark:text-zinc-400 block mb-1">Connection</label>
              <select
                value={conn}
            ...
            <div className="flex-1 min-w-0">
              <label className="text-xs text-zinc-500 dark:text-zinc-400 block mb-1">Function / agent name</label>
              <input
                value={agentName}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <label className="text-xs text-zinc-500 dark:text-zinc-400 block mb-1">Parameters (JSON)</label>
            <textarea
              value={paramsText}
              onChange={(e) => setParamsText(e.target.value)}
              rows={5}
              className="w-full rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-zinc-400"
            />
          </div>
          ...
          {/* Live flow */}
          {(jobId || jobState) && (
            <div className="border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 space-y-4 bg-zinc-50 dark:bg-zinc-800/50/50">
              ...
              <div className="flex items-center gap-2 pt-1">
                <div className={`h-2 w-2 rounded-full ${
                  status === "approved" ? "bg-green-50 dark:bg-green-950/300" :
                  status === "rejected" ? "bg-red-50 dark:bg-red-950/300" :
                  status === "auto_approved" ? "bg-green-400" :
                  isPending ? "bg-blue-50 dark:bg-blue-950/300 animate-pulse" :
                  "bg-zinc-400"
                }`} />
                <span className="text-sm font-medium capitalize text-zinc-700 dark:text-zinc-300 dark:text-zinc-600">
                  {status?.replace(/_/g, " ")}
                </span>
                ...
                <div className="border-t border-zinc-200 dark:border-zinc-700 pt-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={deciding}
                      className="flex-1 border-red-200 text-red-600 hover:bg-red-50 dark:bg-red-950/30"
                    >
              ...
              {isDone && !isPending && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-zinc-600 dark:text-zinc-400"
                >
      ...
      <Card className="mt-6">
        ...
        <CardContent className="space-y-4">
          {savedId ? (
            <div className="space-y-3">
              <div className="flex items-start gap-3 rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/30 px-4 py-3">
                <CheckCircle2 className="h-5 w-5 text-green-600 mt-0.5 shrink-0" />
                <div className="flex-1">
                  <p className="text-sm font-semibold text-green-800 dark:text-green-300">Agent created with unique API key!</p>
...
kit = ApprovalKit(
    ...
    user_id="${saveAgentName || 'my-agent'}",
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-zinc-500 dark:text-zinc-400 block mb-1">Agent name *</label>
                  <input
                    value={saveAgentName}
                    onChange={(e) => setSaveAgentName(e.target.value)}
                    placeholder="My Shopping Bot"
                    className="w-full rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-zinc-400"
                  />
                </div>
                <div>
                  <label className="text-xs text-zinc-500 dark:text-zinc-400 block mb-1">Description</label>
                  <input
                    value={saveAgentDesc}
              ...
              <div>
                <label className="text-xs text-zinc-500 dark:text-zinc-400 block mb-2">Icon</label>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs transition-colors ${
                        saveAgentIcon === value
                          ...
                          : "border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 dark:bg-zinc-800/50"
                      }`}
                    >
              ...
              {conn && action && (
                <div>
                  <label className="text-xs text-zinc-500 dark:text-zinc-400 block mb-1">First scenario title</label>
                  <input
