    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 mt-2 max-w-2xl text-sm leading-relaxed">
          10 specialized agents across 6 domains. Each has a conversational chat interface
          that triggers real ApprovalKit approval flows via Auth0 Token Vault.
        </p>
        {/* Stat bar */}
        <div className="flex items-center gap-3 mt-4 flex-wrap">
          {[
            { label: "Agents", value: "8", color: "blue" },
            { label: "Token Vault Services", value: "30", color: "emerald" },
            { label: "Approval Models", value: "5", color: "purple" },
            { label: "Sub-Agents", value: "7", color: "amber" },
            { label: "Preset Scenarios", value: "24+", color: "red" },
          ].map(s => (
            <div key={s.label} className={`rounded-lg border border-${s.color}-200 dark:border-${s.color}-800 bg-${s.color}-50/40 dark:bg-${s.color}-950/15 px-3 py-1.5 flex items-center gap-2`}>
              <span className={`text-sm font-bold text-${s.color}-700 dark:text-${s.color}-400`}>{s.value}</span>
      ...
      {/* Setup All Demos — FIRST (must be done before demos work) */}
      <div className="mb-8">
          ...
          <CardContent className="p-5 flex flex-col sm:flex-row items-center gap-5">
            ...
            <div className="flex-1 text-center sm:text-left">
              <h2 className="text-lg font-bold text-zinc-800 dark:text-zinc-200">
                {seeded ? "Demo Data Ready" : "Step 1: Setup Demo Data"}
              </h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
                Creates connections, approvers, and rules for all 10 agents in one click.
              </p>
            </div>
      ...
      <div className="mb-10">
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <Badge variant="default" className="text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800">Recommended</Badge>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {["Customer Incidents", "Security Breaches", "Employee Onboarding", "Fraud Detection", "Product Launches", "Rogue Agent Testing"].map(s => (
                ...
                <div className="flex items-center gap-4 text-xs text-zinc-500 dark:text-zinc-400">
                  ...
                  <span>7 sub-agents</span>
                  <span>Per-tool least privilege</span>
                  <span>24+ preset scenarios</span>
                </div>
              </div>
        ...
        {/* Architecture Docs */}
        <Card
          className="group border border-purple-200 dark:border-purple-800 bg-purple-50/50 dark:bg-purple-950/10 hover:bg-purple-100/50 dark:hover:bg-purple-950/20 hover:shadow-md transition-all cursor-pointer mt-3"
          ...
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-purple-600/80 dark:text-purple-400/70 mt-0.5">
                Deep dive: AI Orchestrator, defense-in-depth, 7 sub-agents, adversarial validation, parallel execution
              </p>
            </div>
      ...
      {/* Specialized Agents — clickable */}
      <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-2">
        Specialized Agents
      </p>
      <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
        Click any agent to chat directly, or use the Orchestrator above to let AI choose the right agents for you.
      </p>
              ...
              {catAgents.map(agent => {
                ...
                return (
                  <div
                    ...
                    className={`group rounded-xl border-l-4 ${CATEGORY_BORDER[category]} border border-zinc-200/60 dark:border-zinc-800/60 p-4 cursor-pointer hover:shadow-md hover:-translate-y-0.5 transition-all duration-200`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {agent.scenarios.slice(0, 3).map((s: any, i: number) => (
                            <span key={i} className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                              i === 0 ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" :
                              i === 1 ? "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400" :
                              "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400"
                            }`}>{i === 0 ? "😊" : i === 1 ? "😠" : "💀"} {s.title || s.badgeLabel}</span>
                          ))}
                        </div>
                      </div>
                    </div>
