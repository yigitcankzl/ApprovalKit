...
export interface StoryStep {
  ...
  approvers?: string[]; // e.g. ["Manager", "CFO"] — each needs separate approve click
}
...
interface StepState {
  ...
  jobId?: string;
  message?: string;
  approvalsGiven?: number; // how many individual approvals received
  approvalsNeeded?: number; // total needed (from approvers array length)
}
...
export function StoryRunner({ story, onDone }: { story: Story; onDone?: () => void }) {
  ...
  const runStep = async (index: number) => {
    ...
    try {
      ...
      if (res.status === "auto_approved" || res.status === "pre_approved") {
        ...
        const needed = step.approvers?.length || 1;
        updateStep(index, { status: "pending", jobId: res.job_id, approvalsNeeded: needed, approvalsGiven: 0 });
  ...
  const handleDecision = async (jobId: string, decision: "approve" | "reject", stepIndex: number) => {
    setDeciding(true);
    const state = stepStates[stepIndex];
    const needed = state.approvalsNeeded || 1;
    const given = (state.approvalsGiven || 0) + 1;

    if (decision === "reject") {
      // Reject immediately
      try { await api.submitDecision(jobId, { decision: "reject" }); } catch {}
      setDeciding(false);
      return; // Poll will pick up the rejection
    }

    if (given < needed) {
      // Not all approvals yet — just update the counter, don't send API call
      updateStep(stepIndex, { approvalsGiven: given });
      setDeciding(false);
      return;
    }

    // Last approval — send the real decision
    try { await api.submitDecision(jobId, { decision: "approve" }); } catch {}
    setDeciding(false);
                    ...
                    {state.status === "pending" && (
                      <div className="mt-2 space-y-2">
                        ...
                        {showApproval && state.jobId && (() => {
                          const approvers = story.steps[i]?.approvers || ["Approver"];
                          const needed = state.approvalsNeeded || 1;
                          const given = state.approvalsGiven || 0;
                          const nextApprover = approvers[given] || approvers[approvers.length - 1];

                          return (
                            <div className="p-3 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20 space-y-2.5">
                              <div className="flex items-center gap-1.5 text-[10px] text-zinc-500 dark:text-zinc-400">
                                <Info className="h-3 w-3 shrink-0" />
                                In production, approvals happen via Auth0 Guardian push notifications. These buttons are for demo purposes.
                              </div>

                              {/* Approver progress */}
                              {needed > 1 && (
                                <div className="flex items-center gap-2">
                                  {approvers.map((name, ai) => (
                                    <div key={ai} className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full ${
                                      ai < given
                                      ...
                                      {ai < given ? <CheckCircle2 className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                                      {name}
                                    </div>
                                  ))}
                                  <span className="text-[10px] text-zinc-400">{given}/{needed}</span>
                                </div>
                              )}

                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => handleDecision(state.jobId!, "approve", i)}
                                  disabled={deciding}
                                ...
                                >
                                  ...
                                  Approve as {nextApprover}
                                </button>
                                <button
                                  onClick={() => handleDecision(state.jobId!, "reject", i)}
                                  disabled={deciding}
...
export const AGENT_STORIES: Record<string, Story[]> = {
  expense: [{
    ...
    steps: [
      { label: "Book flights", description: "$2,800 x 3 engineers = $8,400 total — step-up: Manager + CFO required", connection: "stripe-prod", action: "charge", params: { type: "expense", amount_usd: 8400, category: "travel", description: "AWS re:Invent 2026 — flights for 3 engineers", customer: "travel@company.com" }, expectApproval: true, approvers: ["Manager", "CFO"] },
      { label: "Reserve hotel", description: "$285/night x 4 nights x 3 rooms = $3,420", connection: "stripe-prod", action: "charge", params: { type: "expense", amount_usd: 3420, category: "accommodation", description: "Hotel near Las Vegas Convention Center — 3 rooms, 4 nights", customer: "travel@company.com" }, expectApproval: true, approvers: ["Manager"] },
      { label: "Buy conference tickets", description: "$350 registration x 3 = $1,050", connection: "stripe-prod", action: "charge", params: { type: "expense", amount_usd: 1050, category: "registration", description: "AWS re:Invent 2026 registration x 3", customer: "travel@company.com" }, expectApproval: true, approvers: ["Manager"] },
  ...
  release_manager: [{
    ...
    steps: [
      ...
      { label: "Deploy to production", description: "Push v2.5.0 to production servers", connection: "github-main", action: "deploy", params: { ref: "v2.5.0", environment: "production", service: "api" }, expectApproval: true, approvers: ["Maintainer"] },
  ...
  security_incident: [{
    ...
    steps: [
      ...
      { label: "Lock compromised repo", description: "Prevent any code changes to affected repository", connection: "github-prod", action: "lock_repo", params: { repo: "acme/api", reason: "Suspicious commits detected from compromised account" }, expectApproval: true, approvers: ["Security Lead"] },
      { label: "Revoke production tokens", description: "Nuclear option — revoke all production access tokens. Both CTO and Security Lead must approve.", connection: "github-prod", action: "revoke_tokens", params: { scope: "production", reason: "Confirmed data breach — all production credentials potentially compromised" }, expectApproval: true, approvers: ["CTO", "Security Lead"] },
  ...
  recruitment: [{
    ...
    steps: [
      { label: "Send offer letter", description: "Email formal offer to candidate — HR Manager approval required", connection: "gmail-prod", action: "send_email", params: { type: "offer_letter", recipient: "sarah.chen@email.com", subject: "Offer Letter — Senior Engineer at Acme Corp", salary_usd: 175000, body_preview: "We are excited to offer you the Senior Engineer position." }, expectApproval: true, approvers: ["HR Manager"] },
      { label: "Provision GitHub access", description: "Add to organization as member", connection: "github-prod", action: "add_member", params: { username: "sarahchen", org: "acme", role: "member", system: "github" }, expectApproval: true, approvers: ["IT Manager"] },
  ...
  patient_data: [{
    ...
    steps: [
      { label: "Share with external clinic", description: "Send patient records — Doctor approval required", connection: "google-drive-prod", action: "share", params: { patient_id: "MRN-4821", recipient_name: "City Heart Clinic", recipient_type: "external_clinic", recipient_email: "records@cityheartclinic.com" }, expectApproval: true, approvers: ["Doctor"] },
  ...
  prescription_refill: [{
    ...
    steps: [
      { label: "Refill prescription", description: "Adderall 20mg — controlled substance, Doctor approval required", connection: "gmail-prod", action: "send_email", params: { type: "controlled_refill", medication: "Adderall", dosage: "20mg", patient_id: "MRN-1122", recipient: "pharmacy@hospital.com", subject: "Rx Refill: Adderall 20mg for MRN-1122" }, expectApproval: true, approvers: ["Doctor"] },
      { label: "Adjust dosage", description: "Increase from 20mg to 30mg — Doctor + Pharmacist both must approve", connection: "gmail-prod", action: "send_email", params: { type: "dosage_change", medication: "Adderall", dosage: "30mg", patient_id: "MRN-1122", recipient: "pharmacy@hospital.com", subject: "Dosage Change: Adderall 20mg → 30mg for MRN-1122" }, expectApproval: true, approvers: ["Doctor", "Pharmacist"] },
  ...
  gdpr_request: [{
    ...
    steps: [
      { label: "Delete user data", description: "Remove all personal data — Privacy Officer approval", connection: "github-prod", action: "deploy", params: { type: "gdpr_deletion", subject_email: "john@example.com", scope: "full", is_bulk: false }, expectApproval: true, approvers: ["Privacy Officer"] },
